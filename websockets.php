...
abstract class WebSocketServer {
  ...
  function __construct($addr, $port, $bufferLength = 2048, $serverAddr, $serverPort, $loginUserName, $loginUserKey) {
    ...
    // do handshake with server websocket
    $head = "GET / HTTP/1.1"."\r\n".
        "Upgrade: WebSocket"."\r\n".
        "Connection: Upgrade"."\r\n".
        "Origin: http://$addr"."\r\n".
    ...
    $this->serverSocket = fsockopen($serverAddr, $serverPort, $errno, $errstr, 2);
    fwrite($this->serverSocket, $head);
    $headers = fread($this->serverSocket, 2000);
    //stream_set_timeout($this->serverSocket, 0, 10);
    stream_set_blocking($this->serverSocket, false);
    //fwrite($this->serverSocket, "\x00$data\xff" ) or die('error:'.$errno.':'.$errstr);
    //$wsdata = fread($this->serverSocket, 2000); 
    //echo $wsdata;
   	echo "Connected to Server ".$serverAddr."\n";
   	
   	$this->loginUserId = $loginUserName;
  ...
  abstract protected function processFromServer($message); // Called immediately when the data is recieved.
  abstract protected function process($user,$message); // Called immediately when the data is recieved. 
  abstract protected function connected($user);        // Called after the handshake response is sent to the client.
  abstract protected function closed($user);           // Called after the connection is closed.
  abstract protected function alivetimer();           // Called after 10 seconds has passed
  ...
  public function run() {
    while(true) {
      ...
      // check connected from server socket
      if ($this->serverSocket!=null) {
      	 $status = socket_get_status($this->serverSocket);
      	 if ($status['timed_out']) {
       ...
       if ($this->serverSocket!=null) {
	      ...
	      $msg = trim(fread($this->serverSocket, 1000000), " \t\n\r\0\x0B");
	      if ($msg != "") {
	      	
	      	 $msgList = explode("|||", $msg);
	      	 //print_r($msgList);
	      	 foreach ($msgList as $onemsg) {
	      	 //while (!feof($this->serverSocket)) {
	      	 //	$msg .= trim(fread($this->serverSocket, 50000), " \t\n\r\0\x0B");
	      	 //}
	      	 	if ($onemsg == "") continue;
	      	 	$this->processFromServer($onemsg);
	      	 }
	      }
      }
      ...
      foreach ($read as $socket) {
        if ($socket == $this->master) {
          ...
          echo "kgh1".$numBytes."test\n\n\n";
          if ($numBytes === false) {
          	  $this->disconnect($socket);
          	  $this->stdout("Client Connection timeout. TCP connection lost: " . $socket);
